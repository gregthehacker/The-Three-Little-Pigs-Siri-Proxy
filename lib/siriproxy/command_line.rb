...
class SiriProxy::CommandLine
 ...
 dbh.query("CREATE TABLE `key_stats` (
  ...
  exit $?.exitstatus
    ...
    puts "=== Installing latest code from git://github.com/cedbv/The-Three-Little-Pigs-Siri-Proxy.git [#{@branch}] ==="

    tmp_dir = "/tmp/SiriProxy.install." + (rand 9999).to_s.rjust(4, "0")

    `mkdir -p #{tmp_dir}`
    puts `git clone #{branch_opt} git://github.com/cedbv/The-Three-Little-Pigs-Siri-Proxy.git #{tmp_dir}`  if $?.exitstatus == 0
